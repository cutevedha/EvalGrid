...
# Per-1000-token pricing for common models (input and output cost in USD).
# Update this table when provider pricing changes rather than hunting for inline literals.
...
TOKENS_PER_PRICING_UNIT: int = 1000
DEFAULT_PRICING_MODEL: str = "gpt-3.5-turbo"
...
class TokenCostEvaluator(BaseMetric):
    ...
    def compute(self, test_case: TestCase, actual_output: str, input_tokens: int = None, output_tokens: int = None, model: str = "gpt-3.5-turbo", **kwargs) -> float:
        ...
        if input_tokens is None or output_tokens is None:
            return 0.0
        model_pricing = MODEL_PRICING_TABLE.get(model, MODEL_PRICING_TABLE[DEFAULT_PRICING_MODEL])
        cost = (
